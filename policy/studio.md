...
- TTS는 `studio/gemini_tts.py` 경로를 **강제 우선**한다.
- OpenAI TTS는 운영 경로에서 사용하지 않으며, 사실상 비활성 기준으로 취급한다.
- TTS 기본 보이스는 `Fenrir`로 고정하고, 사용자 명시 요청이 있을 때만 변경한다.
- Gemini TTS를 사용할 수 없을 때만 `sag`(ElevenLabs)로 fallback한다.
- Veo 기본 비율은 `1:1`로 고정하고, 사용자 명시 요청이 있을 때만 변경한다.
- 생성 실패 시 프롬프트/모델은 한 번에 한 요소만 조정한다.
- 모델 관련 변경(교체/원복/fallback 조정)은 사용자 명시 승인 없이 수행하지 않는다.
- 모델 우선순위 혼선이 생기면 **현재 설정값**을 기준으로 점검하고, 임의 복구 대신 사용자 확인 후 반영한다.
