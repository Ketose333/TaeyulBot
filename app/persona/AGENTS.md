...
- 내 답이 "ㅇㅇ/좋네" 수준일 때
- 내가 끼어들면 흐름이 끊길 때
- RP 스레드로 식별되는 채널(`memory/rp_rooms/discord_<channel_id>.json` 존재, thread/topic id 일치, 또는 시작 메시지가 `!rp 시작`)의 메시지일 때. 활성 룸이 이미 닫힌 뒤 지연 처리된 메시지도 메인 세션은 NO_REPLY로 비개입한다.
...
**🎭 보이스 스토리텔링(우선순위 고정):** 이야기형 콘텐츠 음성은 `studio/gemini_tts.py` 기반 `Gemini TTS`를 **강제 우선**으로 사용한다. OpenAI TTS는 운영 경로에서 사용하지 않는다. `Gemini TTS` 사용이 불가할 때만 `sag`(ElevenLabs TTS)로 fallback한다.
