...
STALE_CACHE_PATH = ROOMS_DIR / '_stale_cache.json'
...
def _cleanup_stale_cache_for_room(rid: str) -> None:
    cache = _load_json(STALE_CACHE_PATH)
    if not cache:
        return
    if rid in cache:
        del cache[rid]
        _save_json(STALE_CACHE_PATH, cache)
...
def end_room(ctx: Ctx) -> tuple[bool, str]:
    ...
    _cleanup_stale_cache_for_room(room.get('id') or room_id(ctx))
...
def _find_stale_rp_channel_sessions(stale_channel_ids: set[str]) -> list[str]:
...
def cleanup_non_active_rooms() -> dict[str, Any]:
    ...
    if cache:
        ...
        if new_cache != cache:
            _save_json(STALE_CACHE_PATH, new_cache)
    ...
    # 구형 점검 시 과거 RP 스레드 채널 세션 흔적도 함께 리포트
    ...
    stale_session_candidates = _find_stale_rp_channel_sessions(stale_channel_ids)
    ...
    return {
        ...
        'staleChannelSessionCount': len(stale_session_candidates),
        'staleChannelSessions': stale_session_candidates,
        'removedCount': 0,
        'removedFiles': [],
    }
